...
//    Map<UUID, put inside own class {name: "John Doe", jokes[t, t, f, t], proverbs[f,t,f,t], processing methods}>
    ...
    static void setMode(String newMode) {
        ...
        switch (newMode.toLowerCase()) {
            case "proverb":
                AsynchJokeServer.mode = ServerMode.PROVERBMODE;
                break;
            case "joke":
                AsynchJokeServer.mode = ServerMode.JOKEMODE;
            default:
                break;
        }
    }
    ...
    static String getRandomByMode(UserState userState) {
        if (AsynchJokeServer.mode.equals(ServerMode.JOKEMODE)){
    ...
    public static void main(String args[]) throws IOException {
        ...
        System.out.println
                ("Alex Jin's Asynchronous JokeServer server 1.8 starting up, listening at port " + port + "\n");
        ...
        while (controlSwitch) {
            ...
            if (args.length > 0) {
                for (int i = 0; i < args.length; i++) {
                    try {
                        new Thread(new Worker(new DatagramSocket(Integer.parseInt(args[i])), 70)).start();
                    }
                }
            }
...
class AsynchWorker extends Thread {       // Java utils has multi-threading library. Also Worker is a static class inside Server
    ...
    private static int udpPort = 4545;             //store default port of client
    ...
    AsynchWorker (DatagramSocket ds, int t) {
        wait = true;
        size = new byte[512];
        time = t;
    ...
    public void run() { // when a new worker is instantiated with its own socket and then run, we need to get in and out of the stream
        ...
        try {
            ...
            if (!AsynchJokeServer.hashMap.containsKey(clientID)){
                // if not then...
                // -------- Build new state from UUID <<<
                clientState = new UserState(userName, clientID);
            // -------- Add state to hashmap <<<
                AsynchJokeServer.hashMap.put(clientID, clientState);
...
//            printRandomByMode(outToClient, clientFileOutput, clientID);
    ...
    private static void printFirstJoke (PrintStream out) {
        try {
            out.println(AsynchJokeServer.getFirstJoke());
        } catch (Exception e) {
    ...
    private static void printFirstByMode (PrintStream out, FileOutputStream log) {
        try {
            ...
            out.println(AsynchJokeServer.getFirstByMode());
    ...
    private static void printRandomByMode(PrintStream out) {
        ...
        out.println(clientState.getName() + ": " + AsynchJokeServer.getRandomByMode(clientState));
    }
    ...
    private void printNextByUUID(UserState clientState, PrintStream out, FileOutputStream log ){
        ...
        String randomUnseenJoke = AsynchJokeServer.getRandomUnseenByMode(clientState);
    ...
    private void saveUpdatedStateToHashMap(UUID uuid, UserState state) {
        ...
        AsynchJokeServer.hashMap.put(uuid, state);
    }
    ...
    private UserState getUserState(UUID uuid) {
        return AsynchJokeServer.hashMap.get(uuid);
    }
...
class AdminHandler extends Thread {
    ...
    public void run()
    {
        ...
        try{
            ...
            out.println("AdminHandler: Current Server Mode = " + AsynchJokeServer.mode + "\n");

            String modeSelection = in.readLine();

            AsynchJokeServer.setMode(modeSelection);
